...
Não identificamos, no levantamento realizado, iniciativa governamental — no Brasil ou no exterior — que publique em conjunto os dados públicos de origem, o código da reconstrução e uma conferência explícita, capazes de reproduzir seus próprios demonstrativos fiscais oficiais. Se houver, temos interesse em conhecer.
...
**Regras como código: Catala e OpenFisca.** O [Catala](https://catala-lang.org), desenvolvido no Inria (França), é o parente *conceitual* mais próximo da abordagem adotada aqui. É uma linguagem específica de domínio concebida para *literate programming* legislativo: cada parágrafo do texto normativo é anotado com o código que o formaliza, e o compilador extrai tanto o programa executável quanto um documento legível por juristas. Foi desenhado em colaboração com professores de direito, incorpora a *lógica default* — a estrutura de regra geral com exceções que atravessa a redação legislativa — como recurso de primeira classe, e a correção de suas etapas centrais de compilação foi formalmente provada com o assistente de provas F\*. Nasceu de trabalho com a DGFiP, a administração tributária francesa, sobre o motor de cálculo do imposto de renda; a formalização das prestações familiares francesas em Catala revelou um erro na implementação oficial.

O Catala é um instrumento bem mais rigoroso do que o que usamos aqui — Quarto não é compilador verificado e R não é linguagem jurídica. O MDF tem a mesma estrutura de regra geral com exceções, mas nós a tratamos com lógica condicional comum, sem qualquer pretensão de equivalência formal. A premissa, essa sim, é a mesma, e vale enunciá-la: **a fidelidade entre regra e código deve ser legível pelo especialista do domínio, não aceita sob palavra de quem implementou.**

A diferença está no objeto. O Catala formaliza o *direito material* — quem faz jus a quê — para calcular o resultado de um caso concreto. Aqui se operacionalizam *normas de direito financeiro de natureza procedimental*: as regras de agregação e reporte do MDF, aplicadas a uma execução orçamentária já ocorrida, para reconstruir um demonstrativo agregado já publicado. O [OpenFisca](https://openfisca.org), também de origem francesa e hoje usado em vários continentes, está no mesmo eixo: codifica a *legislação* para *simular* políticas, não para reproduzir demonstrativos a partir da execução real.

**Reproducible Analytical Pipelines (Reino Unido).** É o parente mais próximo do ponto de vista *operacional*. Criado em 2017 pelo Department for Culture, Media and Sport com o Government Digital Service e depois difundido pelo Government Statistical Service, o RAP respondeu a um problema que reconhecemos de imediato: estatísticas oficiais produzidas por processos manuais lentos e propensos a erro, com forte dependência de planilhas e software proprietário. Seus princípios centrais — minimizar etapas manuais, ferramentas de código aberto, controle de versão como trilha de auditoria, documentação e garantia de qualidade embutidas, revisão por pares — estão alinhados ao padrão que este repositório procurou atender.

A diferença de escopo merece precisão, porque é estreita. O RAP trata de como o produtor constrói *o próprio* pipeline de forma reprodutível, e diversas estatísticas britânicas já publicam o código desse pipeline. O que se acrescenta aqui é a segunda metade — o que chamamos, ao longo deste documento, de **reconstrução independente**:

- **Reprodutibilidade (RAP):** os mesmos dados, o mesmo código e o mesmo pipeline levam ao mesmo resultado.
- **Reconstrução independente (aqui):** dado público de execução, mais os critérios normativos, mais uma implementação escrita do zero, levam ao resultado que o governo publicou.

A segunda não depende de reproduzir o pipeline interno do produtor. Está mais próxima de uma auditoria computacional independente do que da replicação de uma análise.

Um traço do arranjo brasileiro reforça esse ponto. As fontes de dados — Siga Brasil, mantido pelo Senado Federal, e SOF, no Ministério do Planejamento — são institucionalmente distintas de quem publica o demonstrativo, o Tesouro Nacional, no Ministério da Fazenda. A reconstrução atravessa, portanto, uma fronteira institucional, e a conferência funciona como teste de consistência entre o que uma instituição divulga e o que outra publica.

A experiência britânica também ensina sobre os limites da tecnologia isolada: a revisão de 2021 do Office for Statistics Regulation trata sobretudo de barreiras de adoção — ferramentas, treinamento, tempo, capacidade e apoio organizacional — e não da abordagem em si.
...
- **Despesa com pessoal (RGF Anexo 1) — escopo parcial.** Apura-se apenas a despesa *liquidada*. Os Restos a Pagar Não Processados, que integram a despesa com pessoal para fins do limite da LRF, não entram no cálculo; por conta dessa exclusão, a base aqui apurada é mais estreita que a Despesa Total com Pessoal (DTP) tal como definida para o limite, e por isso não se apresenta nem a razão DTP/RCL nem a apuração de cumprimento dos arts. 19 e 20. A comparação é de valores, contra a parcela liquidada publicada no Diário Oficial. Os RP Não Processados estão disponíveis na base pública: a extensão é imediata a partir do mesmo pipeline.
